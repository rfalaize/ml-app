...
export class MnistModel {
  ...
  async train() {
    ...
    for (let i = 0; i < this.TRAIN_BATCHES; i++) {
      ...
      if (i % this.TEST_ITERATION_FREQUENCY === 0) {
        ...
        validationData = [
          testBatch.xs.reshape([this.TEST_BATCH_SIZE, 28, 28, 1]),
          testBatch.labels
        ];
      }
      ...
      this.subscribers.forEach(element => {
        try {
          element.getTrainingUpdate(updateValue);
        } catch {
          console.log("ERR! could not send update to subscriber");
        }
      });
  ...
  async importModel() {
    console.log("importing pre-trained model...");
    var model_path = "./../../assets/static/models/model.json";
    this.model = tf.loadLayersModel(model_path);
    //"./../../assets/static/models/mnist.json"
